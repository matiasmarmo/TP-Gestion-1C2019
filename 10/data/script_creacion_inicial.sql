...
----------- .: RECORRIDO :. ----------------
INSERT ZAFFA_TEAM.Recorrido_Unico (RECORRIDO_CODIGO, ESTADO_RECORRIDO)
SELECT DISTINCT RECORRIDO_CODIGO, 'A'
FROM gd_esquema.Maestra
...
GO
CREATE PROCEDURE ZAFFA_TEAM.sp_borrarReservas(@FECHA_ACTUAL datetime2(3))			
AS			
BEGIN			
DELETE FROM ZAFFA_TEAM.Reserva			
WHERE DATEDIFF(DAY, RESERVA_FECHA, @FECHA_ACTUAL) > 3;			
END			
GO	
...
CREATE PROCEDURE ZAFFA_TEAM.sp_guardarCliente(@CLI_NOMBRE nvarchar(255),
									...
									@CLI_MAIL nvarchar(255),@FECHA_NAC datetime2(3))
	AS
	BEGIN
		INSERT INTO ZAFFA_TEAM.Cliente(CLI_NOMBRE,CLI_APELLIDO,CLI_DNI,CLI_DIRECCION,CLI_TELEFONO,CLI_MAIL,NOMBRE_ROL,CLI_FECHA_NAC)
		VALUES (@CLI_NOMBRE,@CLI_APELLIDO,@CLI_DNI,@CLI_DIRECCION,@CLI_TELEFONO,
		@CLI_MAIL,'Cliente',@FECHA_NAC)
	END
GO
...
CREATE PROCEDURE ZAFFA_TEAM.sp_crearReserva(@CLI_ID int, @VIAJE_ID int,									
								@CRUCERO_ID nvarchar(50), @CABINA_NRO decimal(18,0),	
									@CABINA_PISO decimal(18,0), @FECHA_ACTUAL datetime2(3))
	AS								
	BEGIN								
		INSERT INTO ZAFFA_TEAM.Reserva(RESERVA_CODIGO,RESERVA_FECHA,CLI_ID,VIAJE_ID,CRUCERO_ID,							
		CABINA_NRO,CABINA_PISO)							
		VALUES (coalesce((select max(reserva_codigo) from ZAFFA_TEAM.Reserva),0)+1,@FECHA_ACTUAL,@CLI_ID,@VIAJE_ID,@CRUCERO_ID,@CABINA_NRO,@CABINA_PISO)							
	END								
GO
...
AS
	...
	BEGIN TRY
		...
		SET MOTIVO = @motivo
		WHERE CRUCERO_ID = @crucero_id
		AND DATEPART(DAY,FECHA_ACTUAL) = DATEPART(DAY,@fecha_actual) 
		AND  DATEPART(MONTH,FECHA_ACTUAL) = DATEPART(MONTH,@fecha_actual) 
		AND  DATEPART(YEAR,FECHA_ACTUAL) = DATEPART(YEAR,@fecha_actual)
	END TRY
	BEGIN CATCH
		ROLLBACK TRANSACTION tr
		DECLARE @mensaje VARCHAR(255) = ERROR_MESSAGE()
...
BEGIN
  ...
  WHERE CRUCERO_ID = @crucero_nuevo AND 
  ( FECHA_SALIDA BETWEEN @fecha_salida AND @fecha_llegada ) and
...
AS
	BEGIN TRY
		UPDATE ZAFFA_TEAM.Viaje 
		SET CRUCERO_ID = @crucero_nuevo 
		WHERE CRUCERO_ID = @crucero_viejo
